...
namespace ABFW
{
	public class AutoSetLabels
	{
        ...
        private static void SetFileABLabel(FileInfo fileinfoObj, string rootFileName)
        {
            ...
            //得到AB包名称
            string strABName = GetABName(fileinfoObj,rootFileName);
            //获取资源文件的相对路径
            int tmpIndex = fileinfoObj.FullName.IndexOf("Assets");
            string strAssetFilePath = fileinfoObj.FullName.Substring(tmpIndex);                    //得到文件相对路径Assets为开头
            //给资源文件设置AB名称以及后缀
            ...
            if (fileinfoObj.Extension == ".unity")   //定义AB包的扩展名
            {
